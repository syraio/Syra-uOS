...
#define _is_StartWith_P(s, x)	(_StrCmp_P((char*)s, x) >= 0)
#define _is_Equal_P(s, x)		(_StrCmp_P((char*)s, x) == 0)

#define _Str_Copy(so, si)		strlcpy((char*)so, si, sizeof(so))
#define _Str_Copy_P(so, si)		strlcpy_P((char*)so, si, sizeof(so))
...
int8_t _StrCmp_P(char* s, const char* x);
...
const char* strcpy_char_P(char* so, const char* si, char ch, uint16_t so_size);
...
void _StrAppend_P(char* so, const char* si, uint16_t so_size );
